...
n_tot = 300*1e3; ntot_str = ['_' num2str(n_tot / 1e3)];
f_invivo_v = 0:.05:.5;

dataset_pars.add_orig = false;
dataset_pars.n_mut = 0;
...
for f_invivo = f_invivo_v
    ...
    if dataset_pars.add_invivo || dataset_pars.add_mutate
        ...
        n_invivo = f_invivo * n_tot;
        if n_invivo > size(S_basis, 2)
            n_invivo = size(S_basis, 2);
        end
        % Shuffle and select
        indx = randperm(size(S_basis, 2));
        S_basis = S_basis(:, indx(1:n_invivo));
        T_basis = T_basis(:, indx(1:n_invivo));
    ...
    dataset_pars.n_invivo = size(S_basis, 2);    
    dataset_pars.n_rand = n_tot - size(S_basis, 2);        
    ...
    train_fn = ['train_RI' ntot_str orig_flag '_' num2str(dataset_pars.frac(1), 2) '_' ...
        num2str(dataset_pars.frac(2), 2) '_' num2str(dataset_pars.frac(3), 2) noise_flag '.mat'];
