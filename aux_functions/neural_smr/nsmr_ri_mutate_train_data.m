function [T_mut, S_mut] = nsmr_ri_mutate_train_data( T_in, xps, n_mut, fuzz)
%function [T_mut, S_mut] = nsmr_ri_mutate_train_data( T_in, xps, n_mut, fuzz)
...
% Model bounds
T_lb = [ 0.9*min(T_in(1,:))     0    0.07*1e-9     0.2*1e-9     -0.46    .03     .03     0 ];
T_ub = [ 1.1*max(T_in(1,:))     1    1.33*1e-9      4*1e-9       0.86    .30      1      1 ];
