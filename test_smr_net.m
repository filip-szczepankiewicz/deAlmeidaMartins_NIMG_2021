...
net_name   = 'net_optimized_182183'; % Select trained network 
...
% Normalize signal
eval(['[S_unif, S_norm_cnst] = ' norm_function '( S_unif, inputs{1}, inputs{2});'])
T_unif = T_unif(T_ind, :); % relevant indexes
...
[S_brain, T_brain] = nsmr_train_data_from_m(m, xps);
...
% Normalize signal
eval(['[S_brain, ~] = ' norm_function '( S_brain, inputs{1}, inputs{2});'])
T_brain = T_brain(T_ind, :); % relevant indexes
