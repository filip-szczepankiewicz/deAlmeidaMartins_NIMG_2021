...
namy = 'train_optimized_182183'; % name given to train dataset

% Training data specs
n_tot       = 590*1e3; % n_tot (number of training vectors)
ntot_str    = ['_' num2str(n_tot / 1e3)];
f_brain_v   = .5; % f_brain (fraction of m_brain parameter vectors)
...
indx_norm = xps.te*1e3 < 65 & xps.b*1e-9 < .11; % indx for signal normalization
%
...
for f_brain = f_brain_v
    ...
    for i = 1:n_repeat
        ...
        % Normalize Signal
        [S_train, S_norm_cnst] = nsmr_normalize_signal(S_train, indx_norm, S_norm_method);
        
        % Store signal normalization details in a structure
        dataset_pars.s_norm_pars.function = 'nsmr_normalize_signal';
        dataset_pars.s_norm_pars.function_input{1} = indx_norm;
        dataset_pars.s_norm_pars.function_input{2} = S_norm_method;
        dataset_pars.s_norm_pars.function_output{1} = S_norm_cnst;
        ...
        train_rep{i}.xps          = xps;
        train_rep{i}.S_train      = single(S_train);
        train_rep{i}.T_train      = single(T_train);
    ...
    train_fn = [namy ntot_str orig_flag '_' num2str(dataset_pars.frac(1), 2) '_' ...
